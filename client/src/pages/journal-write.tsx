      ...
      <header className="ody-journal-topbar">
        ...
        <div className="ody-journal-dim-picker">
          {DIMENSION_KEYS.map((dim) => (
            <button key={dim} className={`ody-badge${categoryTags.includes(dim) ? ' gold' : ''}`} onClick={() => toggleCategory(dim)}>
              <DimensionLabel dim={dim} />
            </button>
          ))}
        </div>
        <Link to={journalId ? `/journals/${journalId}` : '/journals'}>
          <button className="ody-icon-btn" aria-label="Close editor">
            <Icon name="x" size={18} />
          </button>
        </Link>
      </header>
